...
public:
	...
	void Reflect(const Vector3D& point, const Vector3D& normal) { Direction += 2 * Vector3D::Dot(normal, -Direction) * normal; Origin = point; UpdateInverseDirection(); }
	...
	{
		double r, c = -Vector3D::Dot(this->Direction, normal);
		...
		Origin = point + (c > 0 ? -0.001 : 0.001) * normal;
	}
